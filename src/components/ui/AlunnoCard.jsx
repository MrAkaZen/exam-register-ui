    ...
    <div className="rounded-[24px] border border-white/10 bg-slate-950 p-5 transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-950/40">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-semibold text-white">{alunno.nome} {alunno.cognome}</div>
            <div className="text-sm text-slate-400">{alunno.email || 'Nessuna email'}</div>
          </div>
        </div>
        <div className="text-sm font-medium text-slate-400">{alunno.matricola ?? '—'}</div>
      </div>
